...
@app.route('/login', defaults={'forced_entry':None})
@app.route('/login/<forced_entry>')
def showLogin(forced_entry=None):
    state = ''.join(random.choice(string.ascii_uppercase + string.digits)
                    for x in xrange(32))
    login_session['state'] = state
    # return "The current session state is %s" % login_session['state']
    return render_template('login.html', STATE=state, forced_entry=forced_entry)
...
def disconnect():
    if 'provider' in login_session:
        ...
        return redirect(url_for('showPublicRestaurants'))
    else:
        flash('You are not logged in to begin with!')
        redirect(url_for('showPublicRestaurants'))
...
@app.route('/')
@app.route('/publicrestaurants/')
def showPublicRestaurants():
    restaurants = session.query(Restaurant).order_by(asc(Restaurant.name))
    return render_template('publicrestaurants.html', restaurants=restaurants)

@app.route('/publicrestaurants/<int:restaurant_id>/publicmenu')
def showPublicMenu(restaurant_id):
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).one()
    items = session.query(MenuItem).filter_by(
        restaurant_id=restaurant_id).all()
